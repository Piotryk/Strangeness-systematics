...
int main () {
	...
	int npart = 0; // liczba cząstek w evencie?
	ifstream files ("in_files_JAM.txt");
	ofstream wynik ("krotnosci_JAM_rec.dat");
	...
	while (files>>filename){
		...
		file_fail_flag = 0;
		if ( !(data>>nieistotne) )
			{cout<<filename<<"  nie działa"<<endl;
			 failed_files++; file_fail_flag = 1;
			 continue;
		 }
		totalfiles++;
		data >> number_of_events_in_file >> nieistotne >> nieistotne >> nieistotne >> nieistotne;

		while(data>>nieistotne){
			if ( nieistotne != "#" ) {
				cout << "Out of synch       event " << number_of_events_in_file << '\t' << nieistotne << endl << "w pliku: " << filename << endl; file_fail_flag = 1; break;
			}
			data >> nieistotne >> npart >> nieistotne >> nieistotne >> parameter >> nieistotne >> nieistotne >> nieistotne;

			for (int j = 0; j < npart; j++){
				data >> nieistotne >> ID
					  >> nieistotne >> mass >> px >> py >> pz >> E >> nieistotne >> nieistotne >> nieistotne >> nieistotne >> nieistotne
					  >> parent;
		...
		if(!file_fail_flag) {cout << "Plik " << filename << " zliczony. eventów było " << number_of_events_in_file << endl;}
		data.close();
	}
